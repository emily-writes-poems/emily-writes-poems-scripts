...
def get_processed_poems(poem_dir):
    poem_file_paths = []
    with os.scandir(poem_dir) as files:
        for file in files:
            if file.name.endswith('.txt') and not file.name.endswith('_ANNOTATED.txt'):
                poem_file_paths.append(os.path.join(poem_dir, file.name))
